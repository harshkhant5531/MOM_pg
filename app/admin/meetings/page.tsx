...
import { getMeetings, cancelMeeting, createMeeting, updateMeeting } from "@/app/actions/meetings";
    ...
    return (
        <div className="space-y-6 max-w-[1400px] mx-auto pb-20">
            ...
            <div className="flex justify-between items-start">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Meeting Schedule</h1>
                    <p className="text-slate-500 text-sm font-medium">Manage and track all organizational meetings</p>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {[
                    { label: "Upcoming", count: meetings.filter(m => !m.IsCancelled && new Date(m.MeetingDate) > new Date()).length, color: "blue" },
                    { label: "Completed", count: meetings.filter(m => !m.IsCancelled && new Date(m.MeetingDate) <= new Date()).length, color: "emerald" },
                    { label: "Cancelled", count: meetings.filter(m => m.IsCancelled).length, color: "slate" },
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {filteredMeetings.map((meeting, idx) => {
                                    ...
                                    const status = meeting.IsCancelled ? 'cancelled' : isPast ? 'completed' : 'pending';
                                        ...
                                        >
                                            <td className="px-8 py-6">
                                                <div className="flex flex-col gap-1.5">
                                                    ...
                                                    <span className={`w-fit px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider border ${status === 'pending' ? 'bg-blue-50 text-blue-500 border-blue-100' :
                                                        status === 'completed' ? 'bg-emerald-50 text-emerald-500 border-emerald-100' :
                        ...
                        >
                            <div className="p-8 border-b border-slate-50 flex items-center justify-between shrink-0">
                                <div>
                                    ...
                                    <p className="text-slate-500 text-sm">
                                        {editingMeeting ? "Update existing meeting schedule" : "Setup a new organizational meeting"}
                                    </p>
                                </div>
                                <button
                                    onClick={() => setIsModalOpen(false)}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 gap-4">
                                            <div className="space-y-1.5">
                                                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Date & Time</label>
                                                <div className="relative">
                                            ...
                                            <div className="space-y-1.5">
                                                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Meeting Type</label>
                                                <div className="relative">
                                            ...
                                            <div className="space-y-1.5">
                                                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Venue</label>
                                                <div className="relative">
                                    ...
                                    <div className="space-y-3 flex flex-col h-full overflow-hidden">
                                        <div className="flex items-center justify-between">
                                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Invite Participants</label>
                                                ...
                                                {staffList.map((staff) => (
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-[10px] uppercase ${formData.participants.includes(staff.StaffID) ? 'bg-blue-500' : 'bg-slate-200'
                                                                }`}>
                                                                {staff.StaffName.charAt(0)}
                                                            </div>
                                                            <div>
                                                                <p className="text-[13px] font-bold leading-none">{staff.StaffName}</p>
                                                                <p className={`text-[11px] mt-0.5 ${formData.participants.includes(staff.StaffID) ? 'text-blue-100' : 'text-slate-400'
                                                                    }`}>
                            ...
                            <div className="p-8 border-t border-slate-50 bg-slate-50/50 flex gap-3 shrink-0">
                                ...
                                <button
                                    ...
                                    disabled={isSubmitting || !formData.description || !formData.date || !formData.typeId || !formData.venueId}
