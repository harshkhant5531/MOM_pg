      ...
      <div className="flex justify-between items-center bg-white p-8 rounded-3xl border border-slate-100 shadow-xl shadow-blue-500/5">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Viewing all scheduled activities for this month
          </p>
        </div>
        <div className="flex gap-4">
          <button
            onClick={prevMonth}
            className="cursor-pointer  p-3 bg-slate-50 rounded-2xl hover:bg-slate-100 text-slate-600 transition-all active:scale-95"
          >
            <ChevronLeft size={24} />
          </button>
          <button
          ...
          {currentMonthMeetings.length === 0 ? (
            ...
            currentMonthMeetings.map((meeting, idx) => (
              <motion.div
                ...
                className={`bg-white p-6 rounded-3xl border border-slate-100 shadow-lg shadow-blue-500/5 group hover:border-blue-400 transition-all cursor-pointer relative overflow-hidden ${
                  meeting.IsCancelled ? "opacity-60 grayscale" : ""
                }`}
              >
                {/* Status indicator */}
                <div
                  className={`absolute top-0 right-0 px-4 py-1.5 rounded-bl-2xl text-[10px] font-black uppercase tracking-widest ${
                    meeting.IsCancelled
                ...
                <div className="flex flex-col h-full">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <p className="text-[12px] font-black text-blue-600 uppercase tracking-widest">
                        {new Date(meeting.MeetingDate).toLocaleDateString()}
                      </p>
                      <h3 className="font-bold text-slate-900 leading-tight group-hover:text-blue-600 transition-colors capitalize">
