...
export default function AttendancePage() {
    ...
    const selectMeeting = (meeting: any) => {
        ...
        setAttendance(meeting.meetingmember.map((m: any) => ({
            staffId: m.StaffID,
        ...
        <div className="space-y-8 max-w-[1400px] mx-auto">
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-500 text-sm font-medium">Mark and manage meeting attendance</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-blue-500/5 overflow-hidden flex flex-col h-[600px]">
                    <div className="p-6 border-b border-slate-50 bg-slate-50/50">
                        <h3 className="font-bold text-slate-900 text-sm uppercase tracking-widest">Select Meeting</h3>
                    </div>
                    <div className="flex-1 overflow-y-auto">
                        {isLoading ? (
                            ...
                            <div className="p-8 text-center text-slate-400 italic">No meetings available.</div>
                        ) : meetings.map((meeting) => (
                            <button
                                key={meeting.MeetingID}
                                onClick={() => selectMeeting(meeting)}
                                className={`w-full p-6 text-left border-b border-slate-50 transition-all hover:bg-slate-50 flex flex-col gap-2 ${selectedMeeting?.MeetingID === meeting.MeetingID ? 'bg-blue-50/50 border-l-4 border-l-blue-600' : ''
                                    }`}
                            >
                                ...
                                <div className="flex items-center gap-3 text-[11px] font-medium text-slate-400">
                                    ...
                                    <span className="px-2 py-0.5 bg-slate-100 rounded text-slate-500">{meeting.meetingtype?.MeetingTypeName}</span>
                                </div>
                            </button>
                        ))}
                    </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    {!selectedMeeting ? (
                        <div className="bg-white rounded-3xl border border-dashed border-slate-200 h-[600px] flex flex-col items-center justify-center text-slate-400 p-8 text-center">
                            ...
                            <p className="max-w-xs text-sm">Please select a meeting from the list on the left to start marking attendance.</p>
                        </div>
                    ) : (
                        <motion.div
                        ...
                        >
                            ...
                            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10">
                                <div>
                                    <h3 className="font-black text-slate-900 text-lg leading-tight uppercase tracking-tight">{selectedMeeting.MeetingDescription}</h3>
                                    <p className="text-blue-600 text-[12px] font-bold">
                                        {attendance.filter(a => a.isPresent).length} of {attendance.length} participants present
                                    </p>
                                </div>
                                <button
                                    onClick={handleSave}
                                    disabled={isSaving}
                                    className="bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white px-6 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 shadow-lg shadow-blue-500/20 transition-all active:scale-95"
                                >
                                ...
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="bg-slate-50/50 border-b border-slate-100">
                                            ...
                                            <th className="px-8 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest text-center">Presence Status</th>
                                            <th className="px-8 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest">Remarks</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-50">
                                        {attendance.map((member) => (
                                            <tr key={member.staffId} className="hover:bg-slate-50/50 transition-all">
                                                <td className="px-8 py-5">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <span className="text-[14px] font-bold text-slate-700">{member.name}</span>
                                                    </div>
                                                </td>
                                                <td className="px-8 py-5">
                                                    <div className="flex items-center justify-center gap-4">
                                                        <button
                                                            ...
                                                            className={`w-24 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${member.isPresent
                                                ...
                                                <td className="px-8 py-5">
                                                    <input
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            setAttendance(prev => prev.map(a => a.staffId === member.staffId ? { ...a, remarks: val } : a));
                                                        }}
