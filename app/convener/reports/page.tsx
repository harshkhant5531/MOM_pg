...
export default function ReportsPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[600px] text-slate-400 gap-4">
        ...
        <span className="text-lg font-bold uppercase tracking-widest text-[10px]">
  ...
  const reportCards = [
    ...
    },
    {
      ...
      value: `${Math.round((reportData?.summary.completed / reportData?.summary.total) * 100) || 0}%`,
      icon: TrendingUp,
      color: "green",
    },
    {
      label: "Total Upcoming",
      value: reportData?.summary.scheduled || 0,
      icon: Calendar,
      color: "indigo",
    },
    {
      ...
      <header className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Deep insights into organizational meeting performance and logistics
          </p>
        </div>
          ...
          <div className="h-64 flex items-end gap-3 px-4">
            {reportData?.monthlyTrends.map((item: any, i: number) => {
              const maxCount = Math.max(
                ...reportData.monthlyTrends.map((t: any) => t.count),
                1,
              );
              ...
              return (
                <div
                  key={i}
                  className="flex-1 bg-slate-100 dark:bg-gray-800 rounded-t-xl group relative cursor-pointer hover:bg-blue-500 transition-all"
          ...
          <div className="relative h-64 flex items-center justify-center">
            <div className="w-48 h-48 rounded-full border-[16px] border-slate-100 dark:border-gray-800 relative">
              ...
              <div
                ...
                style={{
                  transform: `rotate(${(reportData?.statusDistribution[0]?.percentage || 0) * 3.6}deg)`,
                }}
              />
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl font-black text-slate-900 dark:text-white">
                  {reportData?.statusDistribution[0]?.percentage || 0}%
                </span>
                <span className="text-[10px] font-black text-slate-400 uppercase">
                  {reportData?.statusDistribution[0]?.label}
                </span>
              </div>
            </div>
          </div>
            ...
            {reportData?.statusDistribution.map((status: any, idx: number) => (
              <div key={idx} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div
                    className={`w-3 h-3 rounded-full ${status.color === "blue" ? "bg-blue-500" : status.color === "indigo" ? "bg-indigo-500" : "bg-red-500"}`}
                  />
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-gray-800">
              {meetings.map((m) => (
                ...
                >
                  <td className="px-10 py-6 font-bold text-slate-800 dark:text-white capitalize">
                  ...
                  <td className="px-10 py-6 text-center">
                    {m.IsCancelled ? (
                      ...
                      <span className="text-green-500 font-black text-[10px] uppercase tracking-widest">
                        Archived
                      </span>
                    ) : (
