...
import { getDepartments, deleteDepartment, createDepartment, updateDepartment } from "@/app/actions/master-config";
    ...
    return (
        <div className="p-8 pb-20 max-w-[1200px] mx-auto">
            <div className="flex justify-between items-start mb-10">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Departments</h1>
                    <p className="text-slate-500 dark:text-gray-400 font-medium mt-1">Manage organizational units and their identification codes</p>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {isLoading ? (
                    Array(3).fill(0).map((_, i) => (
                        <div key={i} className="h-48 bg-white dark:bg-gray-900 rounded-3xl border border-slate-100 dark:border-gray-800 animate-pulse" />
                    ))
                    ...
                    >
                        <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                            ...
                            <button
                                ...
                                className="p-2 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-xl hover:bg-red-100 transition-colors"
                            >
                                <Trash2 size={16} />
                            </button>
                        </div>
                        ...
                        <div className="w-14 h-14 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                            <Building2 className="text-slate-400 dark:text-slate-500" size={28} />
                        </div>

                        <h3 className="text-xl font-black text-slate-800 dark:text-white capitalize truncate mb-2">{dept.DepartmentName}</h3>
                        ...
                        <div className="mt-8 pt-6 border-t border-slate-50 dark:border-gray-800 flex items-center justify-between">
                            <span className="text-[10px] font-black text-slate-400 dark:text-gray-500 uppercase">System Entity</span>
                            <div className="flex -space-x-2">
                                {dept.staff?.slice(0, 3).map((s: any, i: number) => (
                                    <div key={i} className="w-7 h-7 rounded-full border-2 border-white dark:border-gray-900 bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[9px] font-bold text-slate-500 dark:text-gray-400">
                        ...
                        >
                            <div className="p-8 border-b border-slate-50 dark:border-gray-800 flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-slate-500 text-sm">Configure organizational unit details</p>
                                </div>
                            ...
                            <form onSubmit={handleSubmit} className="p-8 space-y-6">
                                <div className="space-y-1.5">
                                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Department Name</label>
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Department Code</label>
                                    <div className="relative">
